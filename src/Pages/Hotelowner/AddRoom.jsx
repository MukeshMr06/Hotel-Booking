...
const AddRoom = () => {
    ...
    return (
        <form>
            <Title align='left' font='outfit' title='Add Room' subTitle='Fill the details carefully and accurate
    room details, pricing, and amenities,to enhance the user booking experience.' />
            ...
            <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
                {Object.keys(images).map((key) => (
                    <label htmlFor={`roomImage${key}`} key={key}>
                        ...
                        <input type="file" accept='imgae/*' id={`roomImages${key}`} hidden
            ...
            <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
                ...
                <div>
                    <p className='mt-4 text-grya-800'>
                        price <span className='text-xs'>/night</span>
                    </p>
                    <input type="number" placeholder='0' className='border border-gray-300 mt-1
                    rounded p-2 w-24 ' value={inputs.pricePerNight} onChange={(e) => setInputs({ ...inputs, pricePerNight: e.target.value })} />
                </div>
            </div>
            ...
            <button className='bg-blue-500 text-white px-8 py-2 rounded mt-4 cursor-pointer'>
                Add Room</button>
